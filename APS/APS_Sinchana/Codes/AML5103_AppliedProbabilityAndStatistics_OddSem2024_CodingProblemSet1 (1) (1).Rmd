...
dfBatEbola %>% summarise(across(c('Gene.1', 'Gene.2', 'Gene.3', 'Gene.4', 'Gene.5'), mean))
```
...
for (gene in selected_genes){
  ...
  dfBatGene = dfBat %>% filter(.data[[gene]] == TRUE)
...
probInsured=mean(dfPatient$Insurance=='insured')
print(probInsured)
probNotInsured=mean(dfPatient$Insurance=='not insured')
print(probNotInsured)
...
print(A[3, 'ConditionalProbability'])
  
  
A = dfPatient %>% filter(Insurance == 'not insured') %>% group_by(Condition) %>% summarise(n = n()) %>% mutate(ConditionalProbability = ifelse(Condition == 'good' | Condition == 'fair', n / sum(n), 0)) 
print(sum(A$ConditionalProbability))
